...
exports.getAll = async function (callback) {
    await connection.query('SELECT * FROM realisasi_alsus INNER JOIN user on realisasi_alsus.id_user = user.id_user INNER JOIN satwil ON satwil.id_satwil = user.id_satwil  ', callback);
};
exports.getData = async function(fields,callback) {
    await connection.query('SELECT * FROM realisasi_alsus INNER JOIN user on realisasi_alsus.id_user = user.id_user INNER JOIN satwil ON satwil.id_satwil = user.id_satwil  WHERE realisasi_alsus.id_realisasi_alsus = ?',fields, callback);
};
