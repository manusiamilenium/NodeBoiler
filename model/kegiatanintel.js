...
exports.getAll = async function (callback) {
    await connection.query('SELECT * FROM kegiatan_intelijen INNER JOIN jenis_kegiatan_intelijen ON kegiatan_intelijen.jenis_kegiatan_intelijen = jenis_kegiatan_intelijen.id_jenis INNER JOIN user on kegiatan_intelijen.id_user = user.id_user INNER JOIN satwil ON satwil.id_satwil = user.id_satwil  ', callback);
};
exports.getData = async function(fields,callback) {
    await connection.query('SELECT * FROM kegiatan_intelijen INNER JOIN jenis_kegiatan_intelijen ON kegiatan_intelijen.jenis_kegiatan_intelijen = jenis_kegiatan_intelijen.id_jenis  WHERE kegiatan_intelijen.id_kegiatan_intelijen = ?',fields, callback);
};
